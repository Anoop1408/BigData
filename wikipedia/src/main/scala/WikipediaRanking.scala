...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)((acc, wart) => if(mentionsLanguage(lang, wart.text)) acc + 1
    else acc, (acc, aggreg_partition) => acc + aggreg_partition)
  }
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    langs.map(lg => lg -> occurrencesOfLang(lg, rdd)).sortBy(_._2)(Ordering[Int].reverse)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    ...
    val pairs: RDD[(String,WikipediaArticle)] = rdd.flatMap(art => {langs.map(lg => lg -> (if(mentionsLanguage(lg, art.text)) art else null))})
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.mapValues(i => i.size).sortBy(_._2, ascending = false).collect.toList
  }
  ...
  def zipLangWithPoint(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Int)] = {
    rdd.flatMap(art=>{langs.map(lg=>lg->(if(mentionsLanguage(lg, art.text)) 1 else 0))})
  }
  ...
  def main(args: Array[String]): Unit =
    ...
    // println("Hello! The u.data file has " + wikiRdd + " lines.")
